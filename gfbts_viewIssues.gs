function doGet() {
  ...
  var htmlHead = "<head><style type='text/css'>body {font-family: sans-serif;}.container {background-color: #dbdbdb; border: 1px solid #6f6f6f; border-radius: 15px;padding: 0px 25px 10px 25px;}.attr div {display: inline;margin-right: 50px;font-weight: bold;font-size: 14px;}.attr span {font-weight: normal;font-size: 12px;padding: 2px 4px 2px 4px;border-radius: 20px;border-width: 1px;border-style: solid;}.attr-val-severity-5 {border-color: #597700;background-color: #9FD400;color: white;}.attr-val-severity-4 {border-color: #C36200;background-color: #FFCC99;color: black;}.attr-val-severity-3 {border-color: #D46A00;background-color: #FF9933;color: white;}.attr-val-severity-2 {border-color: #600000;background-color: #990000;color: white;}.attr-val-severity-1 {border-color: #000000;background-color: #252525;color: white;}.desc {background-color: #ffffff;border-color: 1px solid #afafaf;margin: 8px 0px 8px 0px;padding: 9px 15px 9px 15px;}</style></head>";
  var htmlHeader = "<body><h1>Issues (" + issues[0] + ")</h1>";  
  Logger.log(htmlHead + "<body>" + htmlHeader + "</body>");
  return HtmlService.createHtmlOutput(htmlHead + "<body>" + htmlHeader + "</body>");
}
    ...
    for (var j = 0; j < projectIssues; j++) {
      ...
      */
      ...
      output = output
		...
		+ '<div>Status: <span class="attr-val-status-open">Open</span></div></div>'				
		+ '<div class="desc">' + issue[4] + '</div>'
        + '<div class="attach"><div class="attach-item">' + issue[5] + '</div><div class="attach-item">' + issue[6] + '</div>'
        + '</div></div>';      
    }
  }
